...
class AddPropertieViewModel( application: Application) : AndroidViewModel(application)  {
    var propertiesService : PropertiesService = RetrofitProvider.createService(application, PropertiesService::class.java)
    fun addPropertie( adress :String, description :String, type:String,price :Double){
        if(AuthService.idUser!=null){
           var p :Propertie = Propertie(null, AuthService.idUser!!,adress,description, Type.valueOf(type),price)
